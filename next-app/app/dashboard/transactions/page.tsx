    ...
    >
      ...
      <div className="flex gap-2">
        {(["all", "deposit", "withdrawal"] as const).map((filter) => (
          ...
          >
            {t(
              `transactions.${filter === "all" ? "all" : filter + "s"}`
            )}
          </button>
        ))}
      </div>
              ...
              {userTransactions.length > 0 ? (
                userTransactions.map((tx, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-surface-500 hidden sm:table-cell">
                      {tx.cryptoNetwork}
                    </td>
          ...
          >
            ...
            >
              <div className="p-6">
                ...
                {selectedAddress &&
                  (() => {
                    const addr = depositAddresses.find(
                    ...
                    return (
                      <div className="bg-surface-50 rounded-xl p-4 mb-4">
                        <p className="text-xs text-surface-400 mb-1">
                          {t("deposit.network")}: {addr.network}
                        </p>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-mono text-surface-700 flex-1 break-all">
                            {addr.address}
                          </p>
                          <button
                            onClick={() =>
                              copyAddress(addr.address, addr.id)
                            }
